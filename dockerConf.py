...
def writetofile(filepath,filename,filecontant,enforce=False):
    ...
    if not os.path.exists(files):
        print("the %s is Not exist,will be new generate" % files)
        createFile(files)
...
def startDocker(jointoken):
    ...
    status,res=execCmd("重新载入 docker 配置文件",cmds,args.test)
    print("重新载入 docker 配置文件[status: %d]: %s" %(status,res))
    cmds = "sudo systemctl restart docker.service"
    status,res=execCmd("重启 docker",cmds,args.test)
    print("重启 docker [status: %d]: %s"%(status,res))
    
    if jointoken is not None and jointoken != "":
        cmds = "sudo docker swarm leave -f"
        status,res = execCmd("join swarm",cmds,args.test)
        print("离开 docker 集群 [status: %d]: %s"%(status,res))
        cmds = "sudo docker swarm join --token %s" %(jointoken)
        status,res = execCmd("join swarm",cmds,args.test)
        print("加入 docker 集群 [status: %d]: %s"%(status,res))
        
def setDockerPM():
    cmds ="sudo nvidia-smi -pm 1"
    status,res=execCmd("设置显卡专业模式",cmds,args.test)
    print("设置显卡专业模式[status: %d]: %s"%(status,res))
def getAddress():
